 ...
 */
public class PowerPotTile extends TileEntityBotanyPot {
    static class InternalInventoryHandler extends ItemStackHandler {
        PowerPotTile tile;

        public InternalInventoryHandler(PowerPotTile tile) {
            super(6);
            this.tile = tile;
        }
    ...
    public PowerPotTile(Config.PotMkConfig tier) {
        super();
        ...
        this.energy = new PotEnergyStorage(this, tier.maxEnergy.get());
        this.energyLazy = LazyOptional.of(() -> this.energy);
    }

    @Override
    public BlockEntityType<?> getType() {
        if (this.tier.tier == Config.PotMk.MK1) return PowerPots.POWER_TILE_MK1.get();
        if (this.tier.tier == Config.PotMk.MK2) return PowerPots.POWER_TILE_MK2.get();
        if (this.tier.tier == Config.PotMk.MK3) return PowerPots.POWER_TILE_MK3.get();
            ...
            if (!this.level.isClientSide) {
                ...
                if (!didAutoHarvest) {
                    for (final ItemStack item : BotanyPotHelper.generateDrop(this.level.random, this.getCrop())) {
                        item.setCount(this.tier.itemsPerOutput.get());
